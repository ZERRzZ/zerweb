...
export class SMenuComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (changes.smenu && !changes.smenu.firstChange) {
      ...
      if (this.select[0] > 0 && this.select[1] > 0) this.reselect.emit([this.smenu[this.select[0] - 1], this.smenu[this.select[0] - 1].list[this.select[1] - 1]]) // 初始化传出, 需保证 select 值大于 0
